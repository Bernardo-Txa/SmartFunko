...
const SUCCESS_MESSAGE = "Se esse e-mail estiver cadastrado, enviaremos um link de acesso.";
const RATE_LIMIT_MESSAGE =
  "Você solicitou muitos links em pouco tempo. Aguarde alguns minutos e tente novamente.";
const GENERIC_ERROR_MESSAGE = "Não foi possível enviar o link de acesso agora. Tente novamente.";
...
export function MagicLinkLoginForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (magicLinkError) {
      if (isRateLimitError(magicLinkError)) {
        setError(RATE_LIMIT_MESSAGE);
        return;
      }

      if (isInvalidEmailError(magicLinkError)) {
        setError("Informe um e-mail válido.");
        return;
      }

      if (isEmailEnumerationSafeError(magicLinkError)) {
  ...
  return (
    <section className="w-full max-w-md rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6">
      <h1 className="text-2xl font-bold text-[var(--foreground)]">Entrar com link mágico</h1>
