...
export default async function AdminCustomerMapReportPage({ searchParams }: Props) {
  await requireOwnerPage("/admin/relatorios/depara");
  const params = await searchParams;
  const report = await new OrderPdfReportService().getCustomerMapReport(params?.competenceId ?? null);
  ...
  return (
    ...
    >
      <div className="grid gap-6">
        <ReportNavigation active="customer_map" />
        <OrderPdfReportPanel basePath="/admin/relatorios/depara" report={report} />
      </div>
    </AdminShell>
  );
}
