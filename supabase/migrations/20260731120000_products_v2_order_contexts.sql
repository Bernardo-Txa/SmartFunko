alter table public.products
  add column if not exists product_type text not null default 'funko_pop';

update public.products
   set product_type = 'funko_pop'
...
alter table public.v2_order_items
  ...
  add column if not exists item_context text not null default 'general';

update public.v2_order_items item
   set product_id = product_variant.product_id,
       supplier_id = product.supplier_id,
       item_context = case
         when product.supplier_id is null then 'general'
         else 'supplier'
       end
  from public.product_variants product_variant
  join public.products product
    on product.id = product_variant.product_id
 where item.product_variant_id = product_variant.id
   and (
     item.product_id is distinct from product_variant.product_id
     or item.supplier_id is distinct from product.supplier_id
     or item.item_context is distinct from case
       when product.supplier_id is null then 'general'
       else 'supplier'
     end
   );
...
alter table public.v2_order_items
  drop constraint if exists v2_order_items_item_context_check;

alter table public.v2_order_items
  add constraint v2_order_items_item_context_check
  check (item_context in ('general', 'supplier'));

create index if not exists v2_order_items_product_id_idx
on public.v2_order_items(product_id);

create index if not exists v2_order_items_supplier_id_idx
on public.v2_order_items(supplier_id);

create index if not exists v2_order_items_context_idx
on public.v2_order_items(item_context);
